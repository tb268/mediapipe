...
#mediapipe使用用
from bvhmaker2 import main as bv
...
class Set_gui():
    def __init__(self, main_window):
        ...
        #matplotlibウインドウ（中央）
        self.frame_plt=tk.Frame(self.main_window)


        #変換ボタン（下）
        self.mbutton=tk.Button(main_window, text="media", width=10, command=self.mediapipe_start)
        self.mbutton.pack(side=tk.BOTTOM)

        #比較ボタン（下）
        self.cbutton=tk.Button(main_window, text="comp", width=10, command=self.comp)
        self.cbutton.pack(side=tk.BOTTOM)

        create_group_plt(self,self.frame_plt)
