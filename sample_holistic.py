#!/usr/bin/env python
# -*- coding: utf-8 -*-
#python3 sample_holistic.py --plot_world_landmark
#xが左右、yが奥行き、zが上下
import copy
import argparse
...
def plot_world_landmarks(
    ...
    for index in waist_index_list:
        ...
        waistpoint.append(point)

    #角度取得＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    #＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    #＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    #右肘角度ーーーーーーーーーーーーーー
    r_hiji=cal_slope(r_armpoint[0],r_armpoint[1],r_armpoint[2])
    #左肘角度ーーーーーーーーーーーーー
    l_hiji=cal_slope(l_armpoint[0],l_armpoint[1],l_armpoint[2])


    #右肩角度ーーーーーーーーーーーーーー
    #腰、肩、腕
    rkata1=cal_slope(r_bodypoint[1],r_bodypoint[0],r_armpoint[1])
    #左肩、肩、腕
    rkata2=cal_slope(shoulderpoint[0],r_bodypoint[0],r_armpoint[1])
    #左肩、肩、腰
    rkata3=cal_slope(shoulderpoint[0],r_bodypoint[0],r_bodypoint[1])
    #左肩角度ーーーーーーーーーーーーー
    #腰、肩、腕
    lkata1=cal_slope(l_bodypoint[1],l_bodypoint[0],l_armpoint[1])
    #右肩、肩、腕
    lkata2=cal_slope(shoulderpoint[1],l_bodypoint[0],l_armpoint[1])
    #右肩、肩、腰
    lkata3=cal_slope(shoulderpoint[1],l_bodypoint[0],l_bodypoint[1])


    #右膝角度ーーーーーーーーーーーーーー
    r_hiza=cal_slope(r_bodypoint[1],r_bodypoint[2],r_bodypoint[3]) 
    #左膝角度ーーーーーーーーーーーーー
    l_hiza=cal_slope(l_bodypoint[1],l_bodypoint[2],l_bodypoint[3])


    #右足角度（太もも）ーーーーーーーーーーーーーー
    #肩、腰、膝
    r_leg1=cal_slope(r_bodypoint[0],r_bodypoint[1],r_bodypoint[2])
    #左腰、腰、膝
    r_leg2=cal_slope(waistpoint[0],r_bodypoint[1],r_bodypoint[2])
    #左腰、腰、肩
    r_leg3=cal_slope(waistpoint[0],r_bodypoint[1],r_bodypoint[0])
    #左足角度（太もも）ーーーーーーーーーーーーー
    #肩、腰、膝
    l_leg1=cal_slope(l_bodypoint[0],l_bodypoint[1],l_bodypoint[2])
    #右腰、腰、膝
    l_leg2=cal_slope(waistpoint[1],l_bodypoint[1],l_bodypoint[2])
    #右腰、腰、肩
    l_leg3=cal_slope(waistpoint[1],l_bodypoint[1],l_bodypoint[0])

    #上半身の曲がり(腰ベクトルと肩ベクトルの角度)
    twist=cal_slope_for_vector(waistpoint[1],waistpoint[0],shoulderpoint[1],shoulderpoint[0])

    #右足首の角度ーーーーーーーーーーーー
    #膝、足首、足先
    r_ancle1=cal_slope(r_bodypoint[2],r_bodypoint[3],r_bodypoint[4])
    #膝、足首、踵
    r_ancle2=cal_slope(r_bodypoint[2],r_bodypoint[3],r_bodypoint[5])
    #左足首の角度ーーーーーーーーーーーー
    #膝、足首、足先
    l_ancle1=cal_slope(l_bodypoint[2],l_bodypoint[3],l_bodypoint[4])
    #膝、足首、踵
    l_ancle2=cal_slope(l_bodypoint[2],l_bodypoint[3],l_bodypoint[5])

    #右手首の角度ーーーーーーーーーーーー
    r_wrist1=cal_slope(r_armpoint[1],r_armpoint[2],r_armpoint[3])
    r_wrist2=cal_slope(r_armpoint[1],r_armpoint[2],r_armpoint[4])
    r_wrist3=cal_slope(r_armpoint[1],r_armpoint[2],r_armpoint[5])
    #左手首の角度ーーーーーーーーーーーー
    l_wrist1=cal_slope(l_armpoint[1],l_armpoint[2],r_armpoint[3])
    l_wrist2=cal_slope(l_armpoint[1],l_armpoint[2],r_armpoint[4])
    l_wrist3=cal_slope(l_armpoint[1],l_armpoint[2],r_armpoint[5])

    #wrist_rotation=[[l_wrist1,l_wrist2,l_wrist3],[r_wrist1,r_wrist2,r_wrist3]]
    #ancle_rotation=[[l_ancle1,l_ancle2],[r_ancle1,r_ancle2]]
    #leg_rotation=[[l_leg1,l_leg2,l_leg3][r_leg1,r_leg2,r_leg3]]
    #kata_rotation=[[lkata1,lkata2,lkata3],[rkata1,rkata2,rkata3]]
    #hiji_rotation=[l_hiji,r_hiji]
    #hiza_rotation=[l_hiza,r_hiza]
    #角度取得終わり＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
    #＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝＝
