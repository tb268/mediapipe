...
class process:
    def __init__(self,plt,ax,frames1,frames2):
        ...
        for motion_1,motion_2,rot1,rot2 in tzip(Motions_1,Motions_2,Motions_rot1,Motions_rot2):
            ...
            self.poseDraw(plt,ax,motion_1,badlist=badlist,pose_color="grey",isMaster=True)
            self.poseDraw(plt,ax,motion_2,badlist=badlist,pose_color="chartreuse",isMaster=False)
    ...
    def PlotAndColor(self,bonelist,ax,badlist,good_color,bad_color,isMaster,isbad):
        #*****************************************************************************
        #
        ...
        for [i,[bone_x, bone_y, bone_z]] in bonelist:
            ...
            if isMaster:
                self.image_list.append(ax.plot(bone_x, bone_y, bone_z,color=good_color)[0])
                continue

            if isbad:
                self.image_list.append(ax.plot(bone_x, bone_y, bone_z,color=bad_color)[0])
                continue
    ...
    #ポーズ描画用
    def poseDraw(self,plt,ax,landmarks,visibility_th=0.5,badlist=[],pose_color="red",isMaster=False):
        ...
        bad_color="red"
        #背骨
        isSpine=self.PlotAndColor(spine_bonelist,ax,badlist,pose_color,bad_color,isMaster,False)
        #鼻
        self.PlotAndColor(nose_bonelist,ax,badlist,pose_color,bad_color,isMaster,isSpine)

        #左上半身
        isUpLeft=self.PlotAndColor(up_left_bonelist,ax,badlist,pose_color,bad_color,isMaster,isSpine)
        #右上半身
        isUpRight=self.PlotAndColor(up_right_bonelist,ax,badlist,pose_color,bad_color,isMaster,isSpine)

        #左下半身
        isDownLeft=self.PlotAndColor(down_left_bonelist,ax,badlist,pose_color,bad_color,isMaster,False)
        #右下半身
        isDownRight=self.PlotAndColor(down_right_bonelist,ax,badlist,pose_color,bad_color,isMaster,False)

        #左指
        self.PlotAndColor(left_finger_bonelist,ax,badlist,pose_color,bad_color,isMaster,isUpLeft)
        #右指
        self.PlotAndColor(right_finger_bonelist,ax,badlist,pose_color,bad_color,isMaster,isUpRight)   

        #つま先かかと
        self.PlotAndColor(right_toesheel_bonelist,ax,badlist,pose_color,bad_color,isMaster,isDownLeft)
        #つま先かかと
        self.PlotAndColor(left_toesheel_bonelist,ax,badlist,pose_color,bad_color,isMaster,isDownRight)
...
def main():
    ...
    ani=ArtistAnimation(fig,images, interval=100, repeat=False)

    #ani=FuncAnimation(fig,main2(plt,ax,frames1,frames2))
    ani.save("./outmovie/animation.mp4", writer="ffmpeg")
